...
### Prototype solution
Use a **MicroPython** powered *ESP32 MicroController* (**ESP32 MCU**) along with a *Vertical Float switch sensor*(**Floater**) and  *Telerivet Text Message Service*(**SMS Gateway**) .
...
MicroPython is fully supported on **ESP32 MCUs** and makes it an excellent choice for such quickly prototyping such project. Implementing Deep-sleep on **ESP32 MCU** using [LoBo MicroPython for ESP32](https://github.com/loboris/MicroPython_ESP32_psRAM_LoBo/wiki/rtc) is very simple, meaning, no need to implement some advanced deep-sleep. This is also a cool feature that preserves energy consumption. 


*This guide assumes that [MicroPython for ESP32 with psRAM support](https://github.com/loboris/MicroPython_ESP32_psRAM_LoBo/wiki) is already installed on the **ESP32 MCU**, **Python 3** installed on your Personal Computer(**PC**) and you have a valid [Telerivet](https://telerivet.com/product/messaging) account with an active Service Plan for Text Messaging.*

*If no MicroPython for ESP32 with psRAM on the **ESP32 MCU** , please check out [Flashing MicroPython on microcontroller](#flashing-mp-on-mc) below.*
...
*To setup a Telerivet account, please visit [Telerivet.com](https://telerivet.com/product/messaging)*
...
### Establish Serial Connection with ESP32 MCU
Connect the **ESP32 MCU** to you **PC** and run the following command to check if the serial drive is already installed.
...
To load a file from the **ESP32 MCU**, run the following command:
```$ ampy --port <SERIAL-PORT> put <FILE_PATH>```


To read a file from the **ESP32 MCU**, run the following command:
...
- **/boot.py** - This file will MicroPython run first when the **ESP32 MCU** is boots or reset.
...
Copy these three files to the **ESP32 MCU**   using  **`ampy`**  **put** command.
i.e on Mac OSX
...
To verify the copied files on the **ESP32 MCU**, run **`ampy`** **get** command to read the files.
i.e on Mac OSX

```$ ampy --port /dev/tty.SLAB_USBtoUART **get** boot.py``` 

- Connect the **Vertical Float Switch** sensor to **Pin4** and **GND** on the **ESP32 MCU**
- Open your Serial Monitor and reset the **ESP32 MCU** or unplug it and plug it back in the PC.
...
However, with less water in the tank or container an SMS should be send to the phone number given in **/settings.py** if the **ESP32 MCU** is connected to the internet, Telerivet setup correctly.
***Ring Magnet should move to the bottom of the Internal Magnetic Reed Switch***

The Serial Monitor will show all log events if the **ESP32 MCU** is connected to your **PC**.
...
> **ESP32 MCU** or unplug it and plug it back in the **PC** to retest resending the SMS.
...
- Add GMS support
- Add encryption & secure Boot

### Debugging
#### Debugging Windows
Connect the **ESP32 MCU** to you **PC**
- Open PUTTY, select `Serial` and select the serial port `COMX`.
- Change the Baud Rate to `115200

Logs from the **ESP32 MCU** should appear on *PUTTY* or *Serial Monitor*. 
Try soft resetting/reboot  by tapping to the reset button on the **ESP32 MCU**
...
### Troubleshooting:
<a name="flashing-mp-on-mc"><h4>Flashing MicroPython on ESP32 MCU</a></h4>
...
Connect the **ESP32 MCU** to your **PC** via USB, and erase the entire flash using **Esptool**.
...
#### Micro-USB cable
Make sure you have a working Micro-USB before connecting the ESP32 MCU to your PC.
This will save you a lot of time as I experienced when connecting ESP32 MCUs, so I advice your to have more that *ONE* Micro-USB cables. 
